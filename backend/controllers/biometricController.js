...
class BiometricController {
            ...
            await query(
                `UPDATE usuarios 
                 SET reset_code = $1, reset_code_expires = $2, reset_code_attempts = 0,
                     reset_id = $3
                 WHERE id = $4`,
                [resetCode, resetExpiry, resetId, user.id]
            );

            // Enviar email CON BREVO
            console.log('📤 Enviando email con Brevo a:', email);
            try {
                const sendSmtpEmail = new SibApiV3Sdk.SendSmtpEmail();
                sendSmtpEmail.subject = 'Restablecimiento de PIN - Código de Verificación';
    ...
    static async resetPINWithCode(req, res) {
        try {
            const { resetId, verificationCode, newPin } = req.body;
            ...
            const userResult = await query(
                `SELECT id, reset_code, reset_code_expires, reset_code_attempts, email, nombre
                 FROM usuarios 
                 WHERE reset_id = $1 AND reset_code IS NOT NULL`,
                [resetId]
            );
            ...
            try {
                const sendSmtpEmail = new SibApiV3Sdk.SendSmtpEmail();
                sendSmtpEmail.subject = 'PIN Restablecido Exitosamente';
                sendSmtpEmail.htmlContent = `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                const result = await brevoApi.sendTransacEmail(sendSmtpEmail);
                console.log('📧 Email de confirmación enviado con Brevo a:', user.email, '- ID:', result.messageId);
            } catch (emailError) {
                console.error('⚠️ Error enviando email de confirmación con Brevo:', emailError);
                // No fallar la operación principal por error de email
            }
